...
function callGitPullService(string $repoId): array
{
    ...
    $ch = curl_init(rtrim($endpoint, '/') . '/git-pull');
    ...
    curl_setopt_array($ch, [
        ...
        CURLOPT_HTTPHEADER => [
            ...
            'User-Agent: KUZAI-GIT-PULL-PHP/0.1',
        ],
    ]);
...
$result = callGitPullService($repoId);

if (!($result['ok'] ?? false)) {
    jsonOut([
        'ok' => false,
        'error' => $result['error'] ?? 'Git pull service failed',
        'repo' => $repoId,

...
function callGitDiffService(string $repoId): array
{
    ...
    $url = rtrim($endpoint, '/') . '/git-diff?repo=' . rawurlencode($repoId);
    $ch = curl_init($url);
    ...
    curl_setopt_array($ch, [
        ...
        CURLOPT_HTTPHEADER => [
            ...
            'User-Agent: KUZAI-GIT-DIFF-PHP/0.1',
        ],
    ]);
...
$result = callGitDiffService($repoId);

if (!($result['ok'] ?? false)) {
    jsonOut([
        'ok' => false,
        'error' => $result['error'] ?? 'Git diff service failed',
        'repo' => $repoId,
...
jsonOut([
    ...
    'cached_diff' => (string) ($response['cached_diff'] ?? ''),
]);

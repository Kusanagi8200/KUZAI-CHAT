...
function callGitPushService(string $repoId): array
{
    ...
    $ch = curl_init(rtrim($endpoint, '/') . '/git-push');
    ...
    curl_setopt_array($ch, [
        ...
        CURLOPT_HTTPHEADER => [
            ...
            'User-Agent: KUZAI-GIT-PUSH-PHP/0.1',
        ],
    ]);
...
$result = callGitPushService($repoId);

if (!($result['ok'] ?? false)) {
    jsonOut([
        'ok' => false,
        'error' => $result['error'] ?? 'Git push service failed',
        'repo' => $repoId,

...
KUZAI est une application web PHP autonome permettant d’utiliser un assistant IA local depuis une interface personnalisée.

Le projet repose sur une logique simple : garder l’inférence, les fichiers, les recherches et les services sous contrôle local. L’application n’est pas une dépendance de KUZCHAT LLM DUO. Elle fonctionne comme une application standalone servie par Apache2.

Le backend LLM est assuré par `llama.cpp` sur `127.0.0.1:8080`. Le modèle utilisé dans cette installation est `qwen3-8b-q5km`, basé sur un fichier GGUF Qwen3-8B quantifié en Q5_K_M.

La recherche web est assurée par SearXNG installé en natif, sans conteneur, exposé uniquement en local sur `127.0.0.1:8888`. L’application PHP interroge SearXNG via `web-search.php`, puis injecte les résultats dans `chat.php` afin d’enrichir le contexte envoyé au modèle local.

Le support d’upload permet d’envoyer des fichiers texte, code, logs ou JSON. Leur contenu est extrait côté serveur puis injecté dans le prompt pour analyse par le LLM.

#### Fonctionnalités validées

```text
Chat local via llama.cpp
Interface PHP personnalisée
API de statut
Upload de fichiers analysables
Recherche web via SearXNG natif
Injection des résultats web dans le prompt
Ajout automatique des sources web dans la réponse
Bouton STOP côté interface
Boutons WEB / UPLOAD / SEND / CLEAR / REMOVE
Container de réponse dynamique
Services gérés par systemd
Validation par lint PHP, curl, jq et apache2ctl configtest
```
...
Vérifier les modules PHP nécessaires.

```bash
php -m | grep -Ei 'curl|json|mbstring'
```

#### Étape 2 — préparation des répertoires KUZAI
...
#### Étape 3 — configuration principale de l’application

Créer `/var/www/html/KUZAI/app/config.php`.
...
#### Étape 4 — interface principale PHP

Créer `/var/www/html/KUZAI/public/index.php`.
...
#### Étape 5 — API de statut

Créer `/var/www/html/KUZAI/public/api/status.php`.
...
#### Étape 6 — API d’upload

Créer `/var/www/html/KUZAI/public/api/upload.php`.
...
#### Étape 7 — API de recherche web

Créer `/var/www/html/KUZAI/public/api/web-search.php`.
...
Créer `/var/www/html/KUZAI/public/api/chat.php`.
...
#### Étape 9 — JavaScript frontend

Créer `/var/www/html/KUZAI/public/assets/js/app.js`.
...
#### Étape 10 — CSS complet

Créer `/var/www/html/KUZAI/public/assets/css/style.css`.
...
#### Étape 11 — service llama.cpp pour Qwen

Créer ou adapter `/etc/systemd/system/llama-server-a.service`.
...
#### Étape 12 — installation native de SearXNG
...
Créer le venv Python et installer les dépendances.
...
#### Étape 13 — configuration SearXNG

Créer `/etc/searxng/settings.yml`.
...
Cette configuration active `json`, désactive le plugin `tracker_url_remover` qui a généré une erreur runtime, et désactive les moteurs bruyants ou non nécessaires dans cette installation : `wikidata`, `ahmia`, `torch`, `karmasearch` et `brave`.

#### Étape 14 — service systemd SearXNG

Créer `/etc/systemd/system/searxng.service`.
...
Tester SearXNG directement.

```bash
curl -s --max-time 25 "http://127.0.0.1:8888/search?q=OpenAI&format=json" | jq '.query, (.results[0] // null)'
```

#### Étape 15 — configuration Apache

L’application est accessible sous `/KUZAI/` si Apache sert `/var/www/html` comme racine par défaut.
...
Si une configuration dédiée est souhaitée, créer `/etc/apache2/conf-available/kuzai.conf`.
...
Si l’application est déjà accessible via `/KUZAI/`, cette configuration dédiée n’est pas obligatoire.

#### Étape 16 — permissions finales
...
#### Étape 17 — validation syntaxique PHP
...
#### Étape 18 — validation Apache, llama.cpp, SearXNG
...
#### Étape 19 — test API status KUZAI
...
#### Étape 21 — test upload de fichier
...
Tester le chat avec le fichier uploadé.
...
#### Étape 23 — test chat avec résultats web injectés
...
2. Vérifier la réponse du modèle.
3. Tester STOP pendant une génération longue.
4. Uploader un fichier .txt ou .log.
5. Demander une analyse du fichier.
6. Écrire une requête web.
7. Cliquer WEB.
8. Vérifier que les sources apparaissent sous le champ.
9. Cliquer SEND.
10. Vérifier que la réponse contient une section Sources.
11. Cliquer CLEAR.
12. Vérifier que l’historique local est vidé.
```

#### Étape 25 — nettoyage des logs SearXNG

Vérifier l’absence d’erreurs récentes.
...
La configuration fournie désactive les moteurs qui avaient généré des erreurs ou des limitations.

#### Étape 26 — backups `.bk` des fichiers critiques

Créer des backups horodatés.
...
#### Étape 27 — validation finale complète
...
L’installation KUZAI fournit une application IA locale exploitable via navigateur, avec :

```text
Interface web PHP standalone
Connexion locale à llama.cpp
Modèle qwen3-8b-q5km
Upload et analyse de fichiers
Recherche web locale avec SearXNG
Injection des sources web dans les réponses
URLs sources ajoutées automatiquement
Contrôle STOP de la génération côté navigateur
Container de réponse dynamique
Services supervisés par systemd
Validation complète par commandes système et API
```

#### Points d’amélioration possibles

```text
Support PDF avec extraction texte
Historique serveur persistant
Mémoire locale par conversation
Indexation documentaire
RAG local
Gestion multi-profils système
Supervision GPU dans l’interface
Authentification HTTP
Journalisation applicative
Export Markdown des conversations
```
